...
namespace EmployeeManagement.Infrastructure.Services
{
    public class EmployeeService
    {
        ...
        public async Task<Employee> CreateEmployee(EmployeeDTO model, string userId, int organizationId)
        {
            try
            {
                ...
                var newUser = await _userRepository.RegisterUserAsync(registerUser);
                if(newUser == null)
                {
                ...
                var roleAllocation = await _roleRepository.AddRole(newUser.UserId, "User");
                if (!roleAllocation)
                {
        ...
        public async Task<Employee> UpdateEmployee(UpdateEmployeeDTO model, int employeeId, string userId)
        {
            try
            {
                ...
                var role = roles[0];
                var updatedEmployee = await _employeeRepository.UpdateEmloyee(model, employeeId, userId, role);
